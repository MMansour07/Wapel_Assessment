                    ...
                    template: function (row) {
                        ...
                        return '\<a onclick="Details(/' + row.Id + '/)" class="btn btn-sm btn-clean btn-icon mr-2" title="Edit details">\
	                        <span class="svg-icon svg-icon-primary svg-icon-2x">\
                                ...
                                \<a onclick="Edit(/' + row.Id + '/)" class="btn btn-sm btn-clean btn-icon mr-2" title="Edit details">\
...
function Edit(Id) {
    window.location.href = "/User/Edit?Id=" + Id.toString().replaceAll("/", "");
}

function Details(Id) {
    window.location.href = "/User/Details?Id=" + Id.toString().replaceAll("/", "") + "&isgeneral="+ true;
}
...
function Delete(Id) {
    Swal.fire({
        ...
        if (result.value) {
            KTApp.blockPage();
            $.ajax({
                url: "/User/DeleteUser?Id=" + Id.toString().replaceAll("/", ""),
                type: "GET",
                dataType: 'json',
                contentType: 'application/x-www-form-urlencoded; charset=UTF-8',
                success: function (response) {
